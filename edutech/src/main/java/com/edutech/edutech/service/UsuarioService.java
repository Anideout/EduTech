...
import com.edutech.edutech.dto.UsuarioPersonaDto;
...
public class UsuarioService {
    ...
    public String asignarPersona(String email, String rut) {
        if (!usuarioRepository.existsById(email)) {
            return "El rut ingresado no existe!";
        } else if (!personaRepository.existsByRut(rut)) {
            ...
            return "usuario: " + usuario.getEmail() + " asignada al usuario: " + persona.getNombre() + " "
                    + persona.getApellido();
        }
    }

    public String asignarPersona(UsuarioPersonaDto dto) {
        if (!usuarioRepository.existsById(dto.getEmail())) {
            return "El rut ingresado no existe!";
        } else if (!personaRepository.existsById(dto.getRut())) {
            return "La especiadad no existe";
        } else {
            Usuario usuario = usuarioRepository.findById(dto.getEmail()).get();
            Persona persona = personaRepository.findById(dto.getRut()).get();

            usuario.setPersona(persona);
            usuarioRepository.save(usuario);

            return "usuario: " + persona.getNombre() + " asignada al profesor: " + persona.getNombre() + " "
                    + persona.getApellido();
        }
    }
     ...
     */
    /*
     * public String asignarusuario(UsuarioPerfilDto dto) {
